...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// Add this boolean variable in your script to avoid MIDI loop in your Components by calling:                                                    //
// components.Component.prototype.midiLoopProtectionEnabled = true;//false; in your script                                                       //
// This will prevent the Component from sending MIDI messages that would cause a loop on the MIDI Port reducing MIDI Signal traffic.             //
// You can can use this metod to avoid MIDI loops in your Components like this:                                                                  //
//                                                                                                                                               //
//  input: function(channel, control, value, status, group) {                                                                                    //
//      if (this.avoidMidiLoop(value ? 1 : 0, "input")) return;                                                                                  //
//        engine.setValue(this.group, this.key, value ? 1 : 0);                                                                                  //  
//  },                                                                                                                                           //
//                                                                                                                                               //
//  output: function(value, group, key) {                                                                                                        //
//      if (this.avoidMidiLoop(value, "output")) return;                                                                                         //
...
(function(global) {
    // Funzione per convertire una stringa in array di chunk SysEx, funziona con caratteri Unicode.
    // Esempio: stringToSysExChunks("🐱") → [[0x01, 0x3e, 0x43, 0x11]]
    function stringToSysExChunks(str) {
        return [...str].map(char => {
            let codePoint = char.codePointAt(0);
            let chunks = [];
            do {
                chunks.unshift(codePoint & 0x7F);
                codePoint >>= 7;
            } while (codePoint > 0);
            return chunks;
        });
    ...
    Component.prototype = {
        ...
        outValueScale: function(outValue) {
            if (outValue > 0.5) {
                return Math.round(outValue * (this.max - 1) + 1);
            } else {
                return Math.round(outValue * (this.max + 1));
            }
        },
        // map output in the XML file, not outValueScale
        output: function(_value, _group, _control) {
            var outputValue = this.outValueScale(this.outGetParameter());
            this.send(outputValue);
        },
        ...
        avoidMidiLoop: function(mode, value, outValue) {
            if (!this.midiLoopProtectionEnabled) return false;

                if (mode === `input`) {
                    if (this._lastInputValue === undefined) {
                        this._lastInputValue = value;
                        return false;
                    };// First time, no loop detected
                    if (value == this.outValueScale(this.outGetValue()) || value == this._lastInputValue) {
                        return true,
                        console.log(`Input loop avoided`);
                    };// If the input value is the same as the value it is supposed to set or as the last input value, avoid loop
                    this._lastInputValue = value;
                } else if (mode === `output`) {
                    if (this._lastOutputValue === undefined) {
                        this._lastOutputValue = outValue;
                        return false;
                    };
                    if (this._lastOutputValue !== undefined && this._lastOutputValue == outValue) {
                        return true,
                        console.log(`Output loop avoided`);
                    };
                    if (this._lastOutputValue !== undefined && this._lastOutputValue !== outValue) {
                        return false;
                    };
    ...
    /**
     * Label Component - Visualizza valori o stati tramite MIDI
     * 
     * Tipi supportati (labelType):
     * - 'LoopValue': Mostra valori frazionari di loop/beatjump (es. "1/2", "4", "16")
     * - 'StatusFeedback': Mostra stato ON/OFF tramite colori (per Invert, LinkType)
     * - 'String': Mostra stringhe di testo (TODO: SysEx personalizzato VDJC)
     */
    const Label = function(options) {
        Component.call(this, options);
        
        // Tipo di label: 'LoopValue', 'StatusFeedback', 'String'
        this.labelType = options.labelType || 'StatusFeedback';
        
        // Colori per StatusFeedback (possono essere sovrascritti nelle options)
        this.colorActive = options.colorActive || 0x7F;
        this.colorInactive = options.colorInactive || 0x00;
    };
    Label.prototype = new Component({
        output: function(value, group, control) {
            // L'output dipende dal tipo di label
            if (this.labelType === 'LoopValue') {
                // TODO: Formattazione valore frazionario per BeatloopSize/BeatjumpSize
                // Placeholder: invia il valore grezzo
                this.send(value);
            } else if (this.labelType === 'StatusFeedback') {
                // Invia colore in base allo stato (ON/OFF)
                const midiValue = (value > 0) ? this.colorActive : this.colorInactive;
                this.send(midiValue);
            } else if (this.labelType === 'String') {
                // TODO: Implementazione futura per SysEx personalizzato VDJC
                console.warn('Label type "String" not yet implemented');
            }
        }
    });
    ...
    const BeatloopToggleButton = function(options) {
        Button.call(this, options);
    };
    BeatloopToggleButton.prototype = new Button ({
        unshift: function() {
    ...
    const ReloopToggleButton = function(options) {
        Button.call(this, options);
    };
    ReloopToggleButton.prototype = new Button({
        unshift: function() {
        ...
        outKey: "reloop_toggle",
    });

    const LoopInButton = function(options) {
        Button.call(this, options);
    };
    LoopInButton.prototype = new Button({
        unshift: function() {
            this.inKey = "loop_in";
        },
        shift: function() {
    ...
    const LoopOutButton = function(options) {
        Button.call(this, options);
    };
    LoopOutButton.prototype = new Button({
        unshift: function() {
            this.inKey = "loop_out";
        },
        shift: function() {
    ...
    const LoopAnchorButton = function(options) {
        Button.call(this, options);
    };
    LoopAnchorButton.prototype = new Button({
        unshift: function() {
            this.inKey = "loop_anchor";
        },
        shift: function() {
            this.inKey = "loop_remove";
        },
        outkey: "loop_anchor",
    });

    const LoopDoubleButton = function(options) {
        Button.call(this, options);
    };
    LoopDoubleButton.prototype = new Button({
        key: "loop_double",
    });

    const LoopHalveButton = function(options) {
        Button.call(this, options);
    };
    LoopHalveButton.prototype = new Button({
        key: "loop_halve",
    });

    const BeatjumpBackwardButton = function(options) {
        Button.call(this, options);
    };
    BeatjumpBackwardButton.prototype = new Button({
        unshift: function() {
    ...
    const BeatjumpForwardButton = function(options) {
        Button.call(this, options);
    };
    BeatjumpForwardButton.prototype = new Button({
        unshift: function() {
    ...
    const BeatjumpSizeDoubleButton = function(options) {
        Button.call(this, options);
    };
    BeatjumpSizeDoubleButton.prototype = new Button({
        key: "beatjump_size_double",
    });

    const BeatjumpSizeHalveButton = function(options) {
        Button.call(this, options);
    };
    BeatjumpSizeHalveButton.prototype = new Button({
        key: "beatjump_size_halve",
    });

    const IntroStartButton = function(options) {
        Button.call(this, options);
    };
    IntroStartButton.prototype = new Button({
        unshift: function() {
    ...
    const IntroEndButton = function(options) {
        Button.call(this, options);
    };
    IntroEndButton.prototype = new Button({
        unshift: function() {
    ...
    const OutroStartButton = function(options) {
        Button.call(this, options);
    };
    OutroStartButton.prototype = new Button({
        unshift: function() {
    ...
    const OutroEndButton = function(options) {
        Button.call(this, options);
    };
    OutroEndButton.prototype = new Button({
        unshift: function() {
    ...
    const HotcueButton = function(options) {
        ...
        this.sendStatusAndType = options.sendStatusAndType || false;
        Button.call(this, options);
    };
    HotcueButton.prototype = new Button({
        ...
        unshift: function() {
            this.input = function(channel, control, value, status, _group) {
                if (this.isPress(channel, control, value, status)) {
                        engine.setValue(this.group, "hotcue_" + this.number + "_activate",1);
                        this.longPressTimer = engine.beginTimer(this.longPressTimeout, () => {
                            engine.setValue(this.group, "hotcue_" + this.number + "_cueloop", 1);
                            this.longPressTimer = NO_TIMER;
                        }, true);
                } else {
                    if (this.longPressTimer !== NO_TIMER) {
        ...
        encodeStatusAndType: function(status, type) {
            // Encode status and type into a single MIDI velocity value
            // status: 0=not set, 1=set, 2=active
            // type: 0=not set, 1=hotcue, 4=saved loop
            
            if (status === 0) return 0; // not set (any type)
            
            if (type === 1) { // hotcue
                return (status === 1) ? 64 : 127; // 64=set, 127=active
            } else if (type === 4) { // saved loop
                return (status === 1) ? 10 : 117; // 10=set, 117=active
            }
        ...
        connect: function() {
            // If sendStatusAndType is enabled, use custom connections instead of default Button connect
            if (undefined !== this.group && this.sendStatusAndType === true) {
                // Track last sent values to avoid duplicates
                this.lastSentStatus = undefined;
                this.lastSentType = undefined;
                
                // Shared function to send MIDI - checks if values changed before sending
                const sendUpdate = function() {
                    const status = engine.getValue(this.group, this.outKey);
                    const type = engine.getValue(this.group, "hotcue_" + this.number + "_type");
                    
                    // Only send if values actually changed
                    if (status !== this.lastSentStatus || type !== this.lastSentType) {
                        const velocity = this.encodeStatusAndType(status, type);
                        // Use throttled sending if available, otherwise fallback to direct MIDI
                        if (typeof sendThrottledHotcueMessage === 'function') {
                            sendThrottledHotcueMessage(this.midi[0], this.midi[1], velocity);
                        } else {
                ...
                // Custom connections for status+type encoding
                this.connections[0] = engine.makeConnection(this.group, this.outKey, sendUpdate);
                this.connections[1] = engine.makeConnection(this.group, "hotcue_" + this.number + "_type", sendUpdate);
                
                // Optional color connection (separate from status/type)
                if (this.colorKey !== undefined) {
                    this.connections[2] = engine.makeConnection(this.group, this.colorKey, function(color) {
                ...
                // Standard Button behavior for non-sendStatusAndType hotcues
                Button.prototype.connect.call(this);
    ...
    const EffectAssignmentButton = function(options) {
        // Usa options.channelGroup per la key
        options.key = "group_" + options.channelGroup + "_enable";
        options.group = "[EffectRack1_EffectUnit" + options.effectUnit + "]";
        Button.call(this, options);
    };
    EffectAssignmentButton.prototype = new Button({
        type: Button.prototype.types.push
    });

    // Nuovo pulsante che legge la key dal group del file XML
    const EffectUnitAssignmentButton = function(options) {
        // Per questo pulsante, il group dal XML contiene la key da usare
        // Questo pulsante viene creato all'interno di un EffectUnit
        if (options.xmlGroup) {
            // Il xmlGroup contiene la key (es: "group_[Channel1]_enable")
            this.fixedKey = options.xmlGroup; // Key che rimane sempre la stessa
            options.key = this.fixedKey;
        } else {
            console.warn("EffectUnitAssignmentButton: xmlGroup è richiesto");
        }
        Button.call(this, options);
    };
    EffectUnitAssignmentButton.prototype = new Button({
        type: Button.prototype.types.toggle
    });
    ...
    // SamplerBank ComponentContainer (this class is similar to Deck, but for SamplerBanks)(VDJC actually don't use it)
    const SamplerBank = function(samplerBankNumbers) {
        if (samplerBankNumbers !== undefined && Array.isArray(samplerBankNumbers)) {
            // These must be unique to each instance,
            // so they cannot be in the prototype.
            this.samplerBankNumbers = samplerBankNumbers;
        } else if (samplerBankNumbers !== undefined && typeof samplerBankNumbers === "number" &&
                Math.floor(samplerBankNumbers) === samplerBankNumbers &&
                isFinite(samplerBankNumbers)) {
            this.samplerBankNumbers = [samplerBankNumbers];
        } else {
            console.warn("ERROR! new SamplerBank() called without specifying any sampler bank numbers");
            return;
        }
        this.currentSamplerBank = "[SamplerBank" + this.samplerBankNumbers[0] + "]";
    };
    SamplerBank.prototype = new ComponentContainer({
        setCurrentSamplerBank: function(newGroup) {
            this.currentSamplerBank = newGroup;
            this.reconnectComponents(function(component) {
                if (component.group === undefined
                      || component.group.search(script.samplerBankRegEx) !== -1) {
                    component.group = newGroup;
                }
              });
            },
        toggle: function() {
            // cycle through samplerBankNumbers array
            let index = this.samplerBankNumbers.indexOf(parseInt(
                script.samplerBankRegEx.exec(this.currentSamplerBank)[1]
            ));
            if (index === (this.samplerBankNumbers.length - 1)) {
                index = 0;
            } else {
                index += 1;
            }
            this.setCurrentSamplerBank("[SamplerBank" + this.samplerBankNumbers[index] + "]");
        }
    });
    ...
    JogWheelBasic.prototype = new Component({
        ...
        inputWheel: function(_channel, _control, value, _status, _group) {
            ...
            const isChannel = this.group.match(script.channelRegEx);
            
            if (isChannel) {
                // Use engine.scratch API for Channels
                ...
                // For Samplers/PreviewDecks: check scratch2_enable directly
                if (engine.getValue(this.group, 'scratch2_enable') > 0) {
                    // Manual scratch calculation (simplified - same as engine.scratchTick)
                    const velocity = (value * this.rpm * Math.PI) / (1 * this.wheelResolution);
                    engine.setValue(this.group, 'scratch2', velocity);
                } else {
                    this.inSetValue(value);
                }
            }
        ...
        inputTouch: function(channel, control, value, status, _group) {
            ...
            if (this.isPress(channel, control, value, status) && this.vinylMode) {
                if (isChannel) {
                    ...
                    // For Samplers/PreviewDecks: use scratch2_enable directly
                    engine.setValue(this.group, 'scratch2_enable', 1);
                }
            } else {
                if (isChannel) {
        ...
        connect: function() {
            ...
            // Extract deck number from different group types
            let match = script.channelRegEx.exec(this.group);  // [ChannelN]
            if (match) {
                this.deck = parseInt(match[1]);
            } else {
                // Try [SamplerN]
                match = this.group.match(/\[Sampler(\d+)\]/);
                if (match) {
                    this.deck = parseInt(match[1]);
                } else {
                    // Try [PreviewDeckN]
                    match = this.group.match(/\[PreviewDeck(\d+)\]/);
                    if (match) {
                        this.deck = parseInt(match[1]);
                    }
                }
    ...
    const EffectUnit = function(unitNumbers, allowFocusWhenParametersHidden, colors) {
        ...
        this.setCurrentUnit = function(newNumber) {
            ...
            this.reconnectComponents(function(component) {
                ...
                // Aggiorna i pulsanti di assegnamento mantenendo la key fissa
                if (component instanceof EffectUnitAssignmentButton) {
                    component.group = eu.group;
                    component.key = component.fixedKey; // Mantiene la key originale
                    component.inKey = component.fixedKey;
                    component.outKey = component.fixedKey;
                }
            });
        };
        ...
        // Container per i pulsanti di assegnamento che usano key personalizzate
        this.unitAssignmentButtons = new ComponentContainer();
        this.unitAssignmentButtons.addButton = function(name, xmlGroup, midi) {
            this[name] = new EffectUnitAssignmentButton({
                group: eu.group,
                xmlGroup: xmlGroup,
                midi: midi
            });
        ...
        this.init = function() {
            ...
            // Inizializza anche i pulsanti di assegnamento personalizzati
            this.unitAssignmentButtons.forEachComponent(function(button) {
                if (button.midi !== undefined) {
                    button.disconnect();
                    button.connect();
                    button.trigger();
    ...
    exports.BeatloopToggleButton = BeatloopToggleButton;
    exports.ReloopToggleButton = ReloopToggleButton;
    exports.LoopInButton = LoopInButton;
    exports.LoopOutButton = LoopOutButton;
    exports.LoopAnchorButton = LoopAnchorButton;
    exports.LoopDoubleButton = LoopDoubleButton;
    exports.LoopHalveButton = LoopHalveButton;
    exports.BeatjumpBackwardButton = BeatjumpBackwardButton;
    exports.BeatjumpForwardButton = BeatjumpForwardButton;
    exports.BeatjumpSizeDoubleButton = BeatjumpSizeDoubleButton;
    exports.BeatjumpSizeHalveButton = BeatjumpSizeHalveButton;
    exports.IntroStartButton = IntroStartButton;
    exports.IntroEndButton = IntroEndButton;
    exports.OutroStartButton = OutroStartButton;
    exports.OutroEndButton = OutroEndButton;
    exports.HotcueButton = HotcueButton;
    exports.SamplerButton = SamplerButton;
    exports.EffectUnitAssignmentButton = EffectUnitAssignmentButton;
    exports.Pot = Pot;
    ...
    exports.NO_TIMER = NO_TIMER;
    exports.stringToSysExChunks = stringToSysExChunks;
